...
# get data location
folder_path = '/Users/az/Desktop/YouTubeTutorials-main/files/scRNAseq_integration/Data/'
dirs <- list.dirs(path = folder_path, recursive = F, full.names = F)

for(x in dirs){
  name <- gsub('_filtered_feature_bc_matrix','', x)
  
  cts <- ReadMtx(mtx = paste0(folder_path,x,'/matrix.mtx.gz'),
          features = paste0(folder_path,x,'/features.tsv.gz'),
          cells = paste0(folder_path,x,'/barcodes.tsv.gz'))
...
merged_seurat_filtered$Patient[merged_seurat_filtered$Patient == 'folder'] <- 'parent'
...
saveRDS(merged_seurat_filtered, file = paste0(folder_path, '/merged_seurat_filtered.rds'))
...
saveRDS (seurat.integrated, file = '/Users/az/Desktop/YouTubeTutorials-main/files/scRNAseq_integration/Analyzed/seurat_integrated_using_cca')
